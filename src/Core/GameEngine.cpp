...
#include <iostream>
#include <iomanip>    // Dùng để định dạng chuỗi điểm số
...
bool GameEngine::Init(const char* title, int xPos, int yPos, int width, int height, bool fullscreen) {
    ...
    m_pMap = new Map();
    m_pPlayer = new Player(new LoaderParams(0, 0, Config::TILE_SIZE, Config::TILE_SIZE, "idle_down"));
...
void GameEngine::LoadLevel(int levelIndex) {
    ...
    // 2. Thiết lập vị trí nhân vật
    MapPoint startPos = m_pMap->GetStartPoint();
    // Chuyển tọa độ lưới (Grid) -> Tọa độ pixel (nhân với kích thước ô lưới)
    m_pPlayer->SetPosition(startPos.col * Config::TILE_SIZE, startPos.row * Config::TILE_SIZE);
...
std::string GameEngine::GetCurrentStoryImageID(const std::string& suffix) const {
    ...
    if (sceneNum <= 3) {
        ...
        // Vòng 4 trở đi: 
        // Với Rank S/A: Ngẫu nhiên hình ảnh của 3 vòng đầu
        if (suffix == "_rankS" || suffix == "_rankA") {
...
void GameEngine::HandleEvents() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_KEYDOWN) {
            ...
            // 1. Phím '1': Về Intro (Không RESET Level)
    ...
    switch (m_currentState) {
        case STATE_PLAY:
            if (m_pPlayer) {
                ...
                int pRow = (int)(m_pPlayer->GetY() / Config::TILE_SIZE);
                int pCol = (int)(m_pPlayer->GetX() / Config::TILE_SIZE);
                ...
                if (m_shrinesCollected == m_totalShrines && !m_isWinningSequence) {
                ...
                if (m_isWinningSequence) {
                    ...
                    if (m_winDelayTimer > 1.0f) {
                        m_randomRankImageID = ""; // Khôi phục ảnh ngẫu nhiên
...
void GameEngine::OnPlayerMove() {
    ...
    SaveState(); // Lưu lại bước đi để Undo
}
...
void GameEngine::SaveState() {
    ...
    // Chuyển pixel -> grid
    moment.playerGridRow = (int)(m_pPlayer->GetY() / Config::TILE_SIZE);
    moment.playerGridCol = (int)(m_pPlayer->GetX() / Config::TILE_SIZE);
...
void GameEngine::Undo() {
    ...
    if (m_currentState == STATE_PLAY) {
        if (m_historyStack.size() > 1) { // Phải còn ít nhất trạng thái Start
            ...
            // Khôi phục Player
            m_pPlayer->SetPosition(prev.playerGridCol * Config::TILE_SIZE, prev.playerGridRow * Config::TILE_SIZE);
    ...
    // 2. Nếu đang ở màn hình khác: Quay về màn hình trước (Undo State)
    else {
...
void GameEngine::HandleStoryInput(SDL_Event& event) {
    if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_RETURN) {
        ...
        switch (m_currentState) {
            ...
            case STATE_RESULT_S:
                // Hạng S -> Chuyển sang màn tiếp theo
                NextLevel();
                break;

            case STATE_RESULT_A:
            case STATE_RESULT_LOSE:
                // Thua hoặc Hạng A -> Chơi lại màn hiện tại
...
void GameEngine::NextLevel() {
    m_currentLevelIdx++;
    // Đã phá đảo toàn bộ game -> Về Intro
    if (m_currentLevelIdx > (int)m_levelFiles.size()) {
        SwitchState(STATE_INTRO); 
        return;
    }

    // Logic vòng 4 trở đi (Index > 3) -> Vào thẳng game, bỏ qua cốt truyện
    if (m_currentLevelIdx > 3) {
        LoadLevel(m_currentLevelIdx);
        SwitchState(STATE_PLAY);
    } else {
        // Vòng 1, 2, 3 -> Hiện cốt truyện
        SwitchState(STATE_STORY_NAME);
    }
}
